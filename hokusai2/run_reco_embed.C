...
)
{
  ...
  cout << "split no. : " << fSplitNo << endl;
  cout << "number of events in split : " << fNumEventsInSplit << endl;
  cout << "number of events in run : " << fNumEventsInRun << endl;
  ...
  TString out = fPathToData+runName+".embed.root";
  ...
  decoder -> SetEventList(fSelectedEventArray);
  ...
  genfitPID -> SetPersistence(false);
  ...
  smallOutput -> SetOutputFile((fPathToData+runName+".conc.root").Data());
